...
std::vector<patternMatrix> removeTransposeDupes(std::string patternSet, std::vector<patternMatrix> patterns) {
    ...
    std::ofstream ptRemovals("deduping2/" + patternSet + "-s1-r1-T-dupes.txt");
    std::ofstream ptRemovalsDetailed("deduping2/" + patternSet + "-s1-r1-T-dupes-detailed.txt");
    ...
    std::ofstream ptNoDupesWithID("deduping2/" + patternSet + "-s1-g1-no-T-dupes-with-id.txt");
    std::ofstream ptNoDupes("deduping2/" + patternSet + "-s1-g1-no-T-dupes.txt");
...
std::vector<patternMatrix> remove23Swaps(std::string patternSet, std::vector<patternMatrix> patterns) {
    ...
    // Remove patterns that are duplicates of a transpose of another pattern
    std::ofstream ptRemovals("deduping2/" + patternSet + "-s2-r2-23-swap-dupes.txt");
    std::ofstream ptRemovalsDetailed("deduping2/" + patternSet + "-s2-r2-23-swap-dupes-detailed.txt");
    for (auto& [psum, bucket] : patternBuckets) {
        //std::cout << "Pattern Sum for this Bucket: " << psum << std::endl;
        //std::cout << "Number of Patterns in this Bucket: " << bucket.size() << std::endl;
        // Compare each pattern to each other pattern in the 2/3 swap sum the bucket
        for (int i = 0; i < bucket.size(); ++i) {
            int b = bucket[i].swap23.zSum;
            bool removeOriginal = false;
            for (int j = 0; j < patternBuckets[b].size(); ++j) {
                // Don't compare to yourself
                if (bucket[i].id == patternBuckets[b][j].id) {
                    //std::cout << "SKIP - Attempted to compare transpose of pattern " << bucket[i].id << " to itself" << std::endl;
                    continue;
                }
                if (bucket[i].is23Swap(patternBuckets[b][j])) {
                    if (bucket[i].id928 != 0 && patternBuckets[b][j].id928 != 0) {
                        std::cout << "Both patterns are in 928: " << bucket[i].id928 << " and " << patternBuckets[b][j].id928 << std::endl;
                    }
                    // Since we need to match against the 928 file, keep any patterns that are in the 928 file
                    if (bucket[i].id928 != 0) {
                        ptRemovalsDetailed << bucket[i] << "; 2/3 Swap = " << bucket[i].swap23 << "; Matches = " << patternBuckets[b][j] << std::endl;
                        ptRemovals << patternBuckets[b][j] << std::endl;
                        patternBuckets[b].erase(patternBuckets[b].begin() + j);
                        --j;
                        continue;  // We need to continue to the next pattern in the bucket as the matching is fuzzy
                    }
                    if (!removeOriginal) {
                        ptRemovalsDetailed << patternBuckets[b][j] << "; 2/3 Swap = " << patternBuckets[b][j].swap23 << "; Matches = " << bucket[i] << std::endl;
                        ptRemovals << bucket[i] << std::endl;
                        removeOriginal = true;
                        continue;
                    }
                    // I don't think we should ever get to this part of the code
                    std::cout << "Original already flagged for removal. I don't think this should happen:" << std::endl;
                    std::cout << "Original: " << bucket[i] << std::endl;
                    std::cout << "Match: " << patternBuckets[b][j] << std::endl;
                }
            }
            if (removeOriginal) {
                bucket.erase(bucket.begin() + i);
    ...
    // Write out the patterns with no duplicates
    std::ofstream ptNoDupesWithID("deduping2/" + patternSet + "-s2-g2-no-23-swap-dupes-with-id.txt");
    std::ofstream ptNoDupes("deduping2/" + patternSet + "-s2-g2-no-23-swap-dupes.txt");
    for(patternMatrix pm : noDupes) {
        ptNoDupesWithID << pm << std::endl;
        ptNoDupes << pm.p << std::endl;
    }
...
std::vector<patternMatrix> remove23SwapTransposes(std::string patternSet, std::vector<patternMatrix> patterns) {
    ...
    // Remove patterns that are duplicates of a transpose of another pattern
    std::ofstream ptRemovals("deduping2/" + patternSet + "-s3-r3-23-swap-T-dupes.txt");
    std::ofstream ptRemovalsDetailed("deduping2/" + patternSet + "-s3-r3-23-swap-T-dupes-detailed.txt");
    for (auto& [psum, bucket] : patternBuckets) {
        //std::cout << "Pattern Sum for this Bucket: " << psum << std::endl;
        //std::cout << "Number of Patterns in this Bucket: " << bucket.size() << std::endl;
        // Compare each pattern to each other pattern in the 2/3 swap sum the bucket
        for (int i = 0; i < bucket.size(); ++i) {
            int b = bucket[i].swap23T.zSum;
            for (int j = 0; j < patternBuckets[b].size(); ++j) {
                // Don't compare to yourself
                if (bucket[i].id == patternBuckets[b][j].id) {
                    //std::cout << "SKIP - Attempted to compare transpose of pattern " << bucket[i].id << " to itself" << std::endl;
                ...
                if (bucket[i].is23SwapT(patternBuckets[b][j])) {
                    if (bucket[i].id928 != 0 && patternBuckets[b][j].id928 != 0) {
                        std::cout << "Both patterns are in 928: " << bucket[i].id928 << " and " << patternBuckets[b][j].id928 << std::endl;
                    }
                    // Since we need to match against the 928 file, keep any patterns that are in the 928 file
                    if (bucket[i].id928 != 0) {
                        ptRemovalsDetailed << bucket[i] << "; 2/3 Swap Transposed = " << bucket[i].swap23T << "; Matches = " << patternBuckets[b][j] << std::endl;
                        ptRemovals << patternBuckets[b][j] << std::endl;
                        patternBuckets[b].erase(patternBuckets[b].begin() + j);
                        break;  // No need to decrement j since we are breaking out of the loop
                    }
                    ptRemovalsDetailed << patternBuckets[b][j] << "; 2/3 Swap Transposed = " << patternBuckets[b][j].swap23T << "; Matches = " << bucket[i] << std::endl;
                    ptRemovals << bucket[i] << std::endl;
                    bucket.erase(bucket.begin() + i);
                    --i;
                    break;
    ...
    // Write out the patterns with no duplicates
    std::ofstream ptNoDupesWithID("deduping2/" + patternSet + "-s3-g3-no-23-swap-T-dupes-with-id.txt");
    std::ofstream ptNoDupes("deduping2/" + patternSet + "-s3-g3-no-23-swap-T-dupes.txt");
    for(patternMatrix pm : noDupes) {
        ptNoDupesWithID << pm << std::endl;
        ptNoDupes << pm.p << std::endl;
    }
...
std::vector<patternMatrix> remove23Swaps(std::string patternSet, std::vector<patternMatrix> patterns) {
    ...
    std::ofstream swap23Removals("deduping2/" + patternSet + "-s2-r2-23-swap-dupes.txt");
    std::ofstream swap23RemovalsDetailed("deduping2/" + patternSet + "-s2-r2-23-swap-dupes-detailed.txt");
...
std::vector<patternMatrix> remove23SwapTransposes(std::string patternSet, std::vector<patternMatrix> patterns) {
    ...
    // Remove patterns that are duplicates of a 2/3 swap of another pattern
    std::ofstream swap23TRemovals("deduping2/" + patternSet + "-s3-r3-23-swap-T-dupes.txt");
    std::ofstream swap23TRemovalsDetailed("deduping2/" + patternSet + "-s3-r3-23-swap-T-dupes-detailed.txt");
...
int main(int argc, char **argv) {
    ...
    std::ofstream step0("deduping2/" + patternSet + "-s0-g0-original-patterns.txt");
    for (patternMatrix pm : patterns2704) {
        step0 << pm << std::endl;
    }
    step0.close();
